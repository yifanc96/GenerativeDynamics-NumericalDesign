...
\midrule
Single-scale FM (1-mask, gauss base)$^{\dagger}$ & 50k & --- & --- & $0.263\pm 0.006$ & $0.115\pm 0.010$ & $0.112\pm 0.014$ & $0.113\pm 0.015$ \\
...
\noindent$^{\dagger}$\emph{We have two independent single-scale FM checkpoints (\texttt{ns\_gauss\_base/}, from \texttt{train\_ns\_gauss\_base.py}; \texttt{ns\_standard\_1mask\_G128/}, from \texttt{train\_ns\_standard.py}). Both are vanilla 1-mask FM with $z_0\sim\cN(0,I)$ and the same UNet architecture (ch=32, dim\_mults=(1,2,2,2)); the standard-1mask version reaches slightly lower numbers (e.g.\ 0.112 vs 0.120 at NFE=64), and we report it as the representative single-scale FM baseline. The two checkpoints differ only by training run / random seed.}
