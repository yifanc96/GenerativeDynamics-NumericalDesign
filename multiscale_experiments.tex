...
Single-scale FM, data-dep & 50k & --- & --- & 0.29 (no err) & 0.14 (no err) & 0.21 (no err) \\
Single-scale meanflow, gauss & 200k & \TBD & \TBD & \TBD & \TBD & \TBD \\
Single-scale meanflow, data-dep & 200k & \TBD & \TBD & \TBD & \TBD & \TBD \\
\midrule
\multicolumn{7}{c}{\textit{Multiscale, mask K=3}} \\
\midrule
Multiscale FM (perband, v1) & 150k & --- & --- & $\mathbf{0.063 \pm 0.003}$ & $0.110\pm 0.003$ & $\mathbf{0.063 \pm 0.004}$ \\
...
\begin{itemize}
\item \textbf{Multiscale FM is the best of the three trained multiscale methods on NS:}\ $\meanrel_{k\leq 60} \approx 0.06$ at NFE=64 or 128. This is the opposite of the GF picture, where flow-map (meanflow / shortcut) clearly beat vanilla FM. On NS, at the available training budgets, neither flow-map method reaches 10\% rel error.
\item Multiscale FM has a peculiar dip / hump pattern: $0.063$ at NFE=16, $0.110$ at NFE=32, then back to $0.063$ at NFE=64. The likely interpretation is that NFE=32 (2 RK4 steps per band) hits a particularly bad RK4 phase, and NFE=64 (4 steps/band) recovers. Worth re-checking with finer NFE grid.
\item Multiscale meanflow plateaus around $0.23$ from NFE=8--32. The mean-flow loss may be insufficiently trained for the more complex NS distribution, or the per-band JVP regularisation interferes with capturing the full enstrophy spectrum.
\item Multiscale shortcut plateaus around $0.15$ across NFE=4..32. The accuracy ceiling is roughly $2\times$ worse than multiscale FM. Shortcut on NS appears more uniform across NFE than meanflow — consistent with the bootstrap loss being more stable but less expressive at this training budget.
\item Single-scale (1-mask) FM and meanflow re-evaluations on NS are \TBD\ (separate eval script; the documented numbers are from training-time eval in the original NS README).
\end{itemize}
