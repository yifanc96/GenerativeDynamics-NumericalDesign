"""
Mean-flow training for 2D Navier-Stokes with standard Gaussian base.

Instead of learning v(z,t) and integrating with many ODE steps,
learns w(z, s, r) that directly transports from time s to r.
At inference, K steps suffice: z_{k+1} = z_k + (r-s) * w(z_k, s_k, r_k).
...
class Logger:
    def __init__(self, config):
        ...
        self.name = f"NS_meanflow_gaussbase_hi{config.hi_size}_lr{config.base_lr}_{self.log_base}"
...
class Trainer:
    ...
    def test_model(self):
        ...
        if 4 in step_counts:
            primary = results['MF4']['band_metrics']['mid'][0]
        else:
...
class Config:
    def __init__(self):
        ...
        self.save_dir = 'results/ns_meanflow_gauss_base'
...
if __name__ == '__main__':
    ...
    p.add_argument('--save_dir', type=str, default='results/ns_meanflow_gauss_base')
