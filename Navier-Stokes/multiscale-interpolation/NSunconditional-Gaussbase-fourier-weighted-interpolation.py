...
class Interpolants:
    ...
    def get_freq_mtx(self):
        nx = self.config.hi_size
        ny = self.config.hi_size

        kx_one_dim = torch.cat([torch.arange(start = 0, end = nx//2),torch.arange(start = -nx//2, end = 0)], dim = 0)
        kx = kx_one_dim.unsqueeze(-1).repeat(1,nx)

        ky_one_dim = torch.cat([torch.arange(start = 0, end = nx//2),torch.arange(start = -nx//2, end = 0)], dim = 0)
        ky = ky_one_dim.unsqueeze(0).repeat(ny,1)

        self.k_mtx = (kx.float()**2 + ky.float()**2).sqrt()
        self.k_mtx[0,0] = 1.0
        self.k_mtx = self.k_mtx[None,None,...]
        self.k_mtx = self.k_mtx.to(self.device)
        print('k_mtx contructed for interpolation with different speed in k')
        return
        
    def freq_alpha(self, t, k_mtx):
        return 1 - (self.wide((1-t)*t)*self.func_freq(self.k_mtx) + self.wide(t))

    def freq_alpha_dot(self, t, k_mtx):
        return -self.func_freq(self.k_mtx)*self.wide(1-2*t) - 1

    def func_freq(self,k_mtx): # 1/|k| - 1
        # return torch.ones_like(k_mtx)/k_mtx.sqrt()-1
        return torch.ones_like(k_mtx)/k_mtx**2-1
    
    def freq_beta(self, t, k_mtx):
        return self.wide((1-t)*t)*self.func_freq(self.k_mtx) + self.wide(t)

    def freq_beta_dot(self, t, k_mtx):
        return self.func_freq(self.k_mtx)*self.wide(1-2*t)+1
    ...
    def It(self, D):
        ...
        aterm = torch.fft.ifftn(self.freq_alpha(t,self.k_mtx) * torch.fft.fftn(z0,dim=(2,3),norm = "forward"),dim=(2,3),norm = "forward").real
        bterm = torch.fft.ifftn(self.freq_beta(t,self.k_mtx) * torch.fft.fftn(z1,dim=(2,3),norm = "forward"),dim=(2,3),norm = "forward").real
    ...
    def Rt(self, D):
        ...
        adotz0 = torch.fft.ifftn(self.freq_alpha_dot(t,self.k_mtx) * torch.fft.fftn(z0,dim=(2,3),norm = "forward"),dim=(2,3),norm = "forward").real
        bdotz1 = torch.fft.ifftn(self.freq_beta_dot(t,self.k_mtx) * torch.fft.fftn(z1,dim=(2,3),norm = "forward"),dim=(2,3),norm = "forward").real
...
class Sampler:
    ...
    def log_wandb_figure(self, sample, D, global_step):
        ...
        if self.config.use_wandb:
            ...
            wandb.log({'training-conditioning_x0_sampledx1_truthx1': wandb.Image(both_train)}, step = global_step)
            wandb.log({'testing-conditioning_x0_sampledx1_truthx1': wandb.Image(both_test)}, step = global_step)
...
class Trainer:
    ...
    def __init__(self, config):
        ...
        # self.scalar_amplitude = (self.spectrum_amplitude**2).mean().sqrt()
        self.scalar_amplitude = torch.zeros_like(self.spectrum_amplitude)
    ...
    def prepare_batch(self, batch, time = 'unif', use_reference_batch = False):
        ...
        noise = torch.randn_like(xhi)
        # tmp = torch.fft.ifftn(self.spectrum_amplitude*noise, dim = (2,3),norm = "forward").real

        tmp = torch.fft.ifftn((self.spectrum_amplitude+self.scalar_amplitude)*noise, dim = (2,3),norm = "forward").real
...
class Loggers:
    ...
    def __init__(self, config):
        ...
        self.verbose_log_name = 'fourieralphabetadiv_k2_matchingvar_GaussODEspectrumnoise_numdata'+ str(config.num_dataset) + 'lag' + str(config.time_lag) + 'noise' + str(config.noise_strength) + 'lo' + str(config.lo_size) + 'hi' + str(config.hi_size) + 'sz' + str(config.base_lr).replace(".","") + 'max' + str(config.max_steps) + '_' + self.log_base
...
class Config:
    def __init__(self,list_data_loc, home = "./"):
        ...
        if self.model_size == 'small':
            ...
            assert False
        self.cond_channels = self.C  # dimension of the conditional channel; here conditioned on z_0
