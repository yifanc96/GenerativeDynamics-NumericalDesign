"""
Evaluate all available standard FM checkpoints for NS spectrum accuracy.
Find which checkpoint has the best spectrum match.
"""
import os, sys, math
import numpy as np
import torch
...
def eval_checkpoint(name, ckpt_path, noise_type, channels, dim_mults,
                    train_data, test_data, device, hi_size=128,
    ...
    print(f"  {'steps':>6}  {'std_ratio':>10}  {'low_enst':>10}  {'mid_enst':>10}  {'high_enst':>10}  {'mean_enst':>10}")
    print(f"  {'-'*68}")
    ...
    for nsteps in step_counts:
        ...
        for bname, mask in bands.items():
            band_err[bname] = np.mean(np.abs(enst_truth[mask] - enst_gen[mask]) / (np.abs(enst_truth[mask]) + 1e-12))
        mean_err = np.mean(np.abs(enst_truth - enst_gen) / (np.abs(enst_truth) + 1e-12))

        print(f"  {nsteps:>6}  {std_ratio:>10.4f}  {band_err['low']:>10.4f}  {band_err['mid']:>10.4f}  {band_err['high']:>10.4f}  {mean_err:>10.4f}")

        all_results[nsteps] = {
            'enst_gen': enst_gen, 'ener_gen': ener_gen,
            'std_ratio': std_ratio, 'band_err': band_err, 'mean_err': mean_err,
        }

    return kvals, enst_truth, ener_truth, all_results
...
if __name__ == '__main__':
    ...
    configs = [
        ...
        ("Gauss-base 5data",    "gauss",    "results/ns_gauss_base_5data/model_final.pt",       32, (1,2,2,2)),
        ...
        ("DataDep 5data",       "data_dep", "results/ns_data_dep_noise_5data/model_final.pt",   32, (1,2,2,2)),
    ]

    all_model_results = {}
    for name, noise_type, ckpt, channels, dim_mults in configs:
        if not os.path.exists(ckpt):
            print(f"\n=== {name}: SKIPPED (no checkpoint at {ckpt}) ===")
            continue
        kvals, enst_truth, ener_truth, results = eval_checkpoint(
            name, ckpt, noise_type, channels, dim_mults,
            train_data, test_data, device, step_counts=step_counts
        )
        all_model_results[name] = (kvals, enst_truth, ener_truth, results)

    # Find best model (lowest mean enstrophy error at 50 steps)
    print(f"\n{'='*70}")
    print("  SUMMARY at 50 RK4 steps:")
    print(f"{'='*70}")
    for name, (kvals, enst_truth, ener_truth, results) in all_model_results.items():
        if 50 in results:
            r = results[50]
            print(f"  {name:<25}  mean_enst={r['mean_err']:.4f}  low={r['band_err']['low']:.4f}  mid={r['band_err']['mid']:.4f}  high={r['band_err']['high']:.4f}  std_ratio={r['std_ratio']:.4f}")

    # Plot spectrum comparison for the best model
    if all_model_results:
        # Pick best at 50 steps
        best_name = min(all_model_results.keys(),
                       key=lambda n: all_model_results[n][3].get(50, {}).get('mean_err', 1e10))
        kvals, enst_truth, ener_truth, results = all_model_results[best_name]

        fig, axes = plt.subplots(1, 2, figsize=(14, 6))

        ax = axes[0]
        ax.loglog(kvals, enst_truth, 'k-', lw=2.5, label='Truth')
        colors = plt.cm.viridis(np.linspace(0.2, 0.9, len(step_counts)))
        for i, ns in enumerate(step_counts):
            if ns in results:
                ax.loglog(kvals, results[ns]['enst_gen'], '--', color=colors[i], lw=1.5,
                         label=f'RK4 {ns} steps (err={results[ns]["mean_err"]:.3f})')
        ax.set_xlabel('Wavenumber k'); ax.set_ylabel('Enstrophy spectrum')
        ax.set_title(f'{best_name} — Enstrophy'); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

        ax = axes[1]
        ax.loglog(kvals, ener_truth, 'k-', lw=2.5, label='Truth')
        for i, ns in enumerate(step_counts):
            if ns in results:
                ax.loglog(kvals, results[ns]['ener_gen'], '--', color=colors[i], lw=1.5,
                         label=f'RK4 {ns} steps')
        ax.set_xlabel('Wavenumber k'); ax.set_ylabel('Energy spectrum')
        ax.set_title(f'{best_name} — Energy'); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

        plt.tight_layout()
        plt.savefig('ns_spectrum_eval_all.png', dpi=200, bbox_inches='tight')
        print(f"\nBest model: {best_name}")
        print(f"Figure saved to ns_spectrum_eval_all.png")
