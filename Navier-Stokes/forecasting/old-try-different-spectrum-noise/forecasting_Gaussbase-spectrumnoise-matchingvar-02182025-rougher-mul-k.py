...
class Trainer:
    ...
    def __init__(self, config):
        ...
        self.spectrum_amplitude = self.spectrum_amplitude * torch.tensor(((np.sqrt(kfreq2D[0]**2 + kfreq2D[1]**2)))).float()
        self.spectrum_amplitude = self.spectrum_amplitude.cuda()
        self.scalar_amplitude = 0.0
        print('[noise] use noise that has a similar Fourier spectrum (multiply with |k| as the vorticity field')
...
class Loggers:
    ...
    def __init__(self, config):
        ...
        self.verbose_log_name = 'specnoisemulk_GaussODE_numdata'+ str(config.num_dataset) + 'lag' + str(config.time_lag) + 'noise' + str(config.noise_strength) + 'lo' + str(config.lo_size) + 'hi' + str(config.hi_size) + 'sz' + str(config.base_lr).replace(".","") + 'max' + str(config.max_steps) + '_' + self.log_base
