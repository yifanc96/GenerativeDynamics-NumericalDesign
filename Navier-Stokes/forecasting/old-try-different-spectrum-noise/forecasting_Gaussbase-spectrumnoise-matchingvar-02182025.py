...
class Loggers:
    ...
    def __init__(self, config):
        ...
        self.verbose_log_name = 'matchingvar_GaussODEcorrectspectrumnoise_numdata'+ str(config.num_dataset) + 'lag' + str(config.time_lag) + 'noise' + str(config.noise_strength) + 'lo' + str(config.lo_size) + 'hi' + str(config.hi_size) + 'sz' + str(config.base_lr).replace(".","") + 'max' + str(config.max_steps) + '_' + self.log_base
