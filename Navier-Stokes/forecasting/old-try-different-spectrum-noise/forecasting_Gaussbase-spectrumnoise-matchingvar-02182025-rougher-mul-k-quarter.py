...
class Trainer:
    ...
    def __init__(self, config):
        ...
        self.spectrum_amplitude = self.spectrum_amplitude * torch.tensor(np.sqrt(np.sqrt(np.sqrt(kfreq2D[0]**2 + kfreq2D[1]**2)))).float()
        self.spectrum_amplitude = self.spectrum_amplitude.cuda()
        self.scalar_amplitude = 0.0
        print('[noise] use noise that has a similar Fourier spectrum (multiply with |k|^1/4 as the vorticity field')
