...
class Interpolants:
    ...
    def beta_dot(self, t):
        return (6*t**2 + 6*t)/(2*t+1)**2 * torch.ones_like(t)
...
class Loggers:
    ...
    def __init__(self, config):
        ...
        self.verbose_log_name = 'zerobasebeta3t2_numdata'+ str(config.num_dataset) + 'lag' + str(config.time_lag) + 'noise' + str(config.noise_strength) + 'lo' + str(config.lo_size) + 'hi' + str(config.hi_size) + 'sz' + str(config.base_lr).replace(".","") + 'max' + str(config.max_steps) + '_' + self.log_base
...
class Config:
    def __init__(self,list_data_loc, home = "./"):
        ...
        self.wandb_project = 'interpolants_forecasting_new' #PLEASE_ENTER_YOUR_OWN_WANDB_ENTITY
        self.wandb_entity = 'yifanc96' #PLEASE_ENTER_YOUR_OWN_WANDB_ENTITY
