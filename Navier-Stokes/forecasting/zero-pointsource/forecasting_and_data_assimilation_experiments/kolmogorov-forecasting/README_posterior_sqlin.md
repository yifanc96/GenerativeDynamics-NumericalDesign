...
## Lag = 40 results (complete, 7/7 configs)
...
| schedule | η = 0.1 | η = 0.5 | η = 1.0 |
|---|---|---|---|
| **follmer**  | 0.479 ± 0.052 | 0.446 ± 0.053 | **0.437 ± 0.043** |
| const    | 0.477 ± 0.071 | 0.454 ± 0.053 | 0.435 ± 0.049 |
| baseline | 0.529 ± 0.072 | 0.477 ± 0.046 | 0.467 ± 0.059 |
| sqrt_t   | 0.574 ± 0.085 | 0.517 ± 0.066 | 0.503 ± 0.061 |
| triangle | 0.609 ± 0.094 | 0.573 ± 0.081 | 0.536 ± 0.066 |
| zero     | 0.802 ± 0.159 | 0.802 ± 0.159 | 0.802 ± 0.159 |

### Natural guidance $\kappa = \eta\,C_g^\text{sqlin}$

| schedule | η = 0.1 | η = 0.5 | η = 1.0 |
|---|---|---|---|
| **follmer**  | 0.478 ± 0.061 | 0.442 ± 0.051 | **0.434 ± 0.034** |
| const    | 0.478 ± 0.044 | 0.450 ± 0.055 | 0.443 ± 0.045 |
| baseline | 0.503 ± 0.054 | 0.466 ± 0.048 | 0.506 ± 0.049 |
| sqrt_t   | 0.524 ± 0.060 | 0.473 ± 0.049 | 0.471 ± 0.043 |
| triangle | 0.562 ± 0.072 | 0.501 ± 0.049 | 0.503 ± 0.039 |
| zero     | 0.651 (ESS=12) | 0.527 (ESS=32) | 0.526 (ESS=24) |

### Lag=40 observations

1. **Föllmer and const tie on RMSE, Föllmer wins on ESS.** At the best config (Doob $\eta=1$): Föllmer 0.437 vs const 0.435 on RMSE (identical), but ESS 24.7 vs 27.2 (comparable). Across all eta values, the two are within 1% of each other — echoing the linlin lag=40 tie.
2. **Guidance pays 10-15%** at lag=40, much more than at lag=10. Going from uncond Föllmer RMSE 0.498 to Doob $\eta=1$ 0.437 is a 12% drop; the observation carries more signal deep in chaos where the prior is broad.
3. **Föllmer = Natural(Föllmer)** to three digits at every $\eta$ (0.479 vs 0.478; 0.446 vs 0.442; 0.437 vs 0.434). $C_g \equiv g^2$ verified again.
4. **`baseline` degrades under Natural $\eta = 1$** (0.467 → 0.506) just like at lag=10 — the same Girsanov $\kappa/g$ amplification hits as $g \to 0$ at $t = 1$.
5. **`zero` Natural-guidance pathology is different at lag=40**: ESS no longer collapses to 1 (shows 12/32 at $\eta=0.1$, 32/32 at $\eta=0.5$) but logZ is $-10^{17}$ and RMSE is worse than `uncond Föllmer`. The particles happen not to diverge numerically, but the weights are effectively uniform on a degenerate proposal.
6. **Order at best-tuned config**: Föllmer ≈ const (0.44) > baseline (0.47) > sqrt_t (0.47–0.50) > triangle (0.50–0.54) > zero (0.53 with natural, 0.80 with doob). Matches linlin lag=40 qualitatively.
