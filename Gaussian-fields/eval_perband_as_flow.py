"""Eval perband_mf_K3 (or shortcut) checkpoints as VANILLA FM (s=r=t, RK4 integration).

The network was trained with flow_ratio=0.5 (50% FM loss with s=r, 50% mean-flow with s<r),
so calling it as net(x, t, t) gives the instantaneous velocity. RK4-integrating that
gives the vanilla flow multiscale result the user remembers.
"""
...
def gen_band_rk4(net, embed_fn, extract_fn, scale, n_samples, nF, device, n_rk4=4):
    """Vanilla FM RK4 — call net(x, t, t) for instantaneous velocity."""
    ...
    for i in range(len(nodes) - 1):
        ...
        def vel(z, tv):
            ...
            inp = embed_fn(z)
            with torch.no_grad():
                pred = net(inp, tt, tt)  # s=r=t -> instantaneous velocity
            return extract_fn(pred)
...
def main():
    ...
    print(f"\n=== {args.ckpt_dir}  treated as vanilla FM (s=r=t, RK4) ===\n")

    for n_rk4 in [1, 2, 4, 8]:
        ...
        for bi, b in enumerate(bands):
            ...
            gen[:, F] = gen_band_rk4(net, ef, xf, scale, args.n_eval, nF, device, n_rk4=n_rk4)
        ...
        nfe = n_rk4 * 4 * (K + 1)
        nb1 = int((rel[:30] < 1).sum()); nb01 = int((rel[:30] < 0.1).sum())
        print(f"RK4/band={n_rk4}  NFE={nfe}: mean_rel<=30={rel[:30].mean():.4f} max={rel[:30].max():.4f} bins<1={nb1}/30 bins<0.1={nb01}/30")
