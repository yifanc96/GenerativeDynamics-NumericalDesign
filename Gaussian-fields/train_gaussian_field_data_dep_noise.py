"""
Flow matching for 2D Gaussian fields with data-dependent noise.
...
def precompute_matern_amplitude(grid_size, sigma_sq, length_scale, s):
    ...
    Matches the notebook: freq scaled by 2*pi*grid_size, zero-mean mode, norm='forward' in ifft."""
...
class Sampler:
    ...
    def _f(self, model, zt, t_val):
        """Evaluate drift at scalar time t_val."""
...
class Logger:
    def __init__(self, config):
        ...
        self.name = f"GaussField_datanoise_s{config.s1}_grid{config.grid_size}_lr{config.base_lr}_{self.log_base}"
...
class Trainer:
    ...
    def get_noise(self, batch_data):
        ...
        if self.prev_batch is None:
            z0 = torch.randn(B, 1, self.config.grid_size, self.config.grid_size, device=self.device)
        else:
    ...
    def fit(self):
        ...
        while self.global_step < cfg.max_steps:
            ...
            t = self.time_dist.sample((cfg.batch_size,)).to(self.device)
    ...
    @torch.no_grad()
    def test_model(self):
        """Evaluate at multiple step counts with both EM and RK."""
        cfg = self.config
        self.model.eval()
        ...
        for nsteps in step_counts:
            ...
            for tag, spec_gen, gen_np in [('RK', spec_rk, gen_rk_np), ('EM', spec_em, gen_em_np)]:
        ...
        for i, ns in enumerate(step_counts):
            ax.loglog(kvals, results[f'RK{ns}']['spec_gen'], '--', color=colors[i], label=f'RK4 {ns}')
        ...
        combined = np.concatenate([truth_np[0], results[f'RK{nmin}']['gen_np'][0], results[f'RK{nmax}']['gen_np'][0]], axis=1)
...
class Config:
    def __init__(self):
        ...
        self.save_dir = 'results/gaussian_field_data_dep_noise'
...
if __name__ == '__main__':
    ...
    p.add_argument('--save_dir', type=str, default='results/gaussian_field_data_dep_noise')
